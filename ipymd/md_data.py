...
class LAMMPS_Data(MD_Data):
    ...
    def _extract_simulation_box(self, f):
        ...
        return np.array([[xlo,0.,0.], [0.,ylo,0.], [0.,0.,zlo],
...
class LAMMPS_Crystal_Data(MD_Data):
    ...
    def get_atom_data(self, atom_style='atomic'):        
        ...
        if atom_style == 'atomic':
            atom_df = pd.DataFrame(atom_data,columns=['id', 'type', 'xs', 'ys', 'zs']).set_index('id')
        elif atom_style == 'charge':
            atom_df = pd.DataFrame(atom_data,columns=['id', 'type', 'q', 'xs', 'ys', 'zs']).set_index('id')
